...
extern "C" {
    ...
    private:
        ...
        // Оцінка 4 послідовні клітинки на полі для певної фішки
        ...
        // Оцінка всього стану ігрового поля для певної фішки
        int scorePosition(const std::vector<std::vector<int>>& board, int piece) {
            int score = 0;

            // Оцінка середнього стовпця
        ...
        // Реалізація алгоритму minimax з альфа бета відсіченням
    ...
    public:
        ...
        // Повертає найближчий порожній рядок у стовпці
        ...
        bool isWinner(const std::vector<std::vector<int>>& board, int piece) {
            ...
            // Діагональні послідовності (позитивний нахил)
            for (int r = 0; r < rows - 3; r++) {
                for (int c = 0; c < cols - 3; c++) {
                    if (board[r][c] == piece &&
                        board[r+1][c+1] == piece &&
            ...
            // Діагональні послідовності (негативний нахил)
        ...
        int getBestMove(const std::vector<std::vector<int>>& currentBoard, int difficulty) {
            ...
            // Простий рівень: випадковий хід
            if (difficulty == EASY) {
                std::uniform_int_distribution<int> dist(0, validMoves.size() - 1);
                return validMoves[dist(rng)];
            }

            // Середній та Важкий рівень використовують minimax з різною глибиною
